...
export const createTicket = (ticket) => {
    return (dispatch, getState, { getFirebase, getFirestore }) => {
        ...
        database.collection('tickets').add({
            ...
            dispatch({ type: actions.CREATE_TICKET, ticket })
        }).catch((err) => {
            dispatch({ type: actions.CREATE_TICKET_ERROR, err })
        })
    }
}
...
export const solveTicket = (ticket, id) => {
    return (dispatch, getState, { getFirebase, getFirestore }) => {
        ...
        database.collection('tickets').doc(id).update({
            ...
            dispatch({ type: actions.SOLVE_TICKET, ticket })
        }).catch((err) => {
            dispatch({ type: actions.SOLVE_TICKET_ERROR, err })
        })
    }
}
